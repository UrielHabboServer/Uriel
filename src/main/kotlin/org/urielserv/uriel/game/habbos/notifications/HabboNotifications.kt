...
class HabboNotifications(
     ...
     */
    suspend fun sendComplexNotification(type: String, vararg args: Pair<String, String>) {
        NotificationDialogMessagePacket(type, *args).send(habbo)
    }

    /**
     * Sends the Habbo a complex notification (wrapper around [sendComplexNotification])
     * This is a more convenient way to send a complex notification
     *
     * @param complexNotification Complex notification to send
     */
    suspend fun sendComplexNotification(complexNotification: ComplexNotification) {
